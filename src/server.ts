...
function faviconResponse(pathname: string): Response | undefined {
  if (pathname === "/favicon.ico") {
    return new Response(Uint8Array.from(atob(FAVICON_ICO_BASE64), (char) => char.charCodeAt(0)), {
      headers: {
  ...
  if (
    ...
    return new Response(Uint8Array.from(atob(FAVICON_PNG_BASE64), (char) => char.charCodeAt(0)), {
      headers: {
        "cache-control": "public, max-age=31536000, immutable",
...
export default {
  async fetch(request: Request, env: unknown, ctx: unknown) {
    const assetResponse = faviconResponse(new URL(request.url).pathname);
